...
// PublicEthereumAPI provides an API to access ESN related information.
// It offers only methods that operate on public data that is freely available to anyone.
type PublicEthereumAPI struct {
	b Backend
}

// NewPublicEthereumAPI creates a new ESN protocol API.
...
// ProtocolVersion returns the current ESN protocol version this node supports
...
// Sign calculates an ESN ECDSA signature for:
...
//
// https://github.com/ethersocial/go-esn/wiki/Management-APIs#personal_sign
...
//
// https://github.com/ethersocial/go-esn/wiki/Management-APIs#personal_ecRecover
func (s *PrivateAccountAPI) EcRecover(ctx context.Context, data, sig hexutil.Bytes) (common.Address, error) {
	if len(sig) != 65 {
		return common.Address{}, fmt.Errorf("signature must be 65 bytes long")
	}
	if sig[64] != 27 && sig[64] != 28 {
		return common.Address{}, fmt.Errorf("invalid ESN signature (V is not 27 or 28)")
	}
...
// PublicBlockChainAPI provides an API to access the ESN blockchain.
// It offers only methods that operate on public data that is freely available to anyone.
type PublicBlockChainAPI struct {
	b Backend
}

// NewPublicBlockChainAPI creates a new ESN blockchain API.
...
// PublicDebugAPI is the collection of ESN APIs exposed over the public
// debugging endpoint.
...
// PrivateDebugAPI is the collection of ESN APIs exposed over the private
// debugging endpoint.
