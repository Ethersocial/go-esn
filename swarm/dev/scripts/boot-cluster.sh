...
#
# The cluster contains a bootnode, a gesc node and multiple swarm nodes, with
...
# start_bootnode starts a bootnode which is used to bootstrap the gesc and
# swarm nodes
start_bootnode() {
...
# start_geth_node starts a gesc node with --datadir pointing at <base-dir>/gesc
# and a single, unlocked account with password "gesc"
start_geth_node() {
  local dir="${base_dir}/gesc"
  mkdir -p "${dir}"
  ...
  if [[ -z "${address}" ]]; then
    fail "failed to get gesc account address"
  fi
  ...
  start_node "gesc" "${GETH_IP}" "$(which gesc)" ${args[@]}
}
...
create_account() {
  ...
  gesc --datadir "${dir}" --password /dev/stdin account new <<< "${password}"
}
