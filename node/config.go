...
type Config struct {
	...
	// used in the devp2p node identifier. The instance name of gesc is "gesc". If no
...
func (c *Config) NodeName() string {
	name := c.name()
	// Backwards compatibility: previous versions used title-cased "Gesc", keep that.
...
// These resources are resolved differently for "gesc" instances.
...
func (c *Config) resolvePath(path string) string {
	...
	// by gesc 1.4 are used if they exist.
	if c.name() == "gesc" && isOldGethResource[path] {
		oldpath := ""
		if c.Name == "gesc" {
