...
func TestConsoleWelcome(t *testing.T) {
	...
	// Start a gesn console, make sure it's cleaned up and terminate the console
	gesn := runGesn(t,
	...
	gesn.Expect(`
Welcome to the Gesn JavaScript console!

instance: Gesn/v{{gesnver}}/{{goos}}-{{goarch}}/{{gover}}
coinbase: {{.Etherbase}}
at block: 0 ({{niltime}})
 datadir: {{.Datadir}}
 modules: {{apis}}
...
func TestIPCAttachWelcome(t *testing.T) {
	...
	if runtime.GOOS == "windows" {
		ipc = `\\.\pipe\gesn` + strconv.Itoa(trulyRandInt(100000, 999999))
	} else {
		ws := tmpdir(t)
		defer os.RemoveAll(ws)
		ipc = filepath.Join(ws, "gesn.ipc")
	}
	...
	testAttachWelcome(t, gesn, "ipc:"+ipc, ipcAPIs)
...
func TestHTTPAttachWelcome(t *testing.T) {
	...
	time.Sleep(2 * time.Second) // Simple way to wait for the RPC endpoint to open
	testAttachWelcome(t, gesn, "http://localhost:"+port, httpAPIs)
...
func TestWSAttachWelcome(t *testing.T) {
	...
	time.Sleep(2 * time.Second) // Simple way to wait for the RPC endpoint to open
	testAttachWelcome(t, gesn, "ws://localhost:"+port, httpAPIs)

	gesn.Interrupt()
	gesn.ExpectExit()
}

func testAttachWelcome(t *testing.T, gesn *testgesn, endpoint, apis string) {
	// Attach to a running gesn note and terminate immediately
	attach := runGesn(t, "attach", endpoint)
	...
	attach.SetTemplateFunc("datadir", func() string { return gesn.Datadir })
